...
void main() {
    vec2 uv = gl_FragCoord.xy * texelSize; // Convert fragment coordinates to texture coordinates
    float aliveCount = 0.0;
    float noiseVal = cnoise(vec4(uv.x * 6.25, uv.y * 6.25, time, 0.0));
	float age = texture2D(currentStateTexture, uv).g;

    // Iterate over the neighbors
    for(int dx=-1; dx<=1; dx++) {
        for(int dy=-1; dy<=1; dy++) {
            if(dx != 0 || dy != 0) { // Exclude the center cell
                vec2 neighborUV = uv + vec2(dx, dy) * texelSize;
                float neighborState = texture2D(currentStateTexture, neighborUV).r;
                aliveCount += neighborState;
            }
        }
    }

    vec4 currentData = texture2D(currentStateTexture, uv);
    float currentState = currentData.r;
    float currentAge = currentData.g;
    float newState;
    float newAge = currentAge;

    if(currentState == 1.0) { // Cell is currently alive
        if(aliveCount < 2.0 || aliveCount > 3.0 && noiseVal < perlinThreshold) newState = 0.0; // Dies
        else {
            newState = 1.0; // Lives
            newAge += 0.01; // Increase age
            newAge = min(newAge, 1.0); // Cap at 1
        }
    } else { // Cell is currently dead
        if ((aliveCount == 3.0) || noiseVal > perlinThreshold) {
            newState = 1.0; // Becomes alive
            newAge = 0.0; // Reset age to 0 for new births
        } else {
            newState = 0.0; // Stays dead
        }
    }
