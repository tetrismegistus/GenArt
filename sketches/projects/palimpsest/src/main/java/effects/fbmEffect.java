...
/**
 * fbmEffect (stackable, ping-pong compliant)
 *
 * Ping-pong contract:
 * - liveCanvas = src (input), set by app via setCanvas(src)
 * - apply(dst, gui) receives dst = output buffer for this stage
 *
 * This effect uses a frozen snapshot by default (consistent with your current design):
 * - onEnable/onResample capture a snapshot from src into frozenInput
 * - shader samples frozenInput
 *
 * If you want it to sample the current stack state every frame instead,
 * replace srcTex = frozenInput with srcTex = liveCanvas (or liveCanvas.get()).
 */
public class fbmEffect extends BaseEffect {
    ...
    public void apply(PGraphics dst, LazyGui gui) {
        ...
        // Use the sketch clock if available; otherwise fall back to 0.
        float time = 0f;
        ...
        if (shader != null) {
            ...
            // IMPORTANT: render into dst, do not in-place filter
            dst.beginDraw();
            dst.shader(shader);
            dst.noStroke();
