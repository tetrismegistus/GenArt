...
void smoothAndDraw(List<Polygon> polygons, color[] p) {
  for (Polygon poly : polygons) {             
      ...
      PShape smoothed = chaikin(shape, .25, 4, true);      
      color randomColor = p[(int) random(p.length)];  
      float r = red(randomColor);
      float g = green(randomColor);
      float b = blue(randomColor);
      color c = color(r, g, b, 175);
      smoothed.setFill(c);                  
      shape(smoothed);
    }
}
